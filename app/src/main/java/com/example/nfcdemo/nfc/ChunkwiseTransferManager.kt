...
class ChunkwiseTransferManager(private val context: Context) {
    ...
    fun updateSettings(maxChunkSize: Int, chunkDelay: Long, transferRetryTimeoutMs: Long) {
        ...
        Log.d(
                TAG,
                "Updated settings: maxChunkSize=$maxChunkSize, chunkDelay=$chunkDelay, transferRetryTimeoutMs=$transferRetryTimeoutMs"
        )
    }
    ...
    fun prepareChunkedMessageSending(message: String): Boolean {
        ...
        Log.d(
                TAG,
                "Prepared chunked message: ${chunksToSend.size} chunks, total length: $messageLength"
        )
        ...
        try {
            ...
            if (acknowledgedChunks.isEmpty()) {
                if (!initializeChunkedTransfer(isoDep)) {
                    ...
                    mainHandler.post {
                        onTransferStatusChanged?.invoke(
                                context.getString(R.string.failed_to_initialize_transfer)
                        )
                    }
            ...
            if (transferResult) {
                ...
                mainHandler.post {
                    onTransferStatusChanged?.invoke(
                            context.getString(R.string.failed_to_complete_transfer)
                    )
                }
                startTransferRetryTimeout()
                return false
    ...
    private fun initializeChunkedTransfer(isoDep: IsoDep): Boolean {
        ...
        mainHandler.post {
            onTransferStatusChanged?.invoke(
                    context.getString(R.string.sending_chunk, 0, totalChunks)
            )
        }
    ...
    private fun sendAllChunks(isoDep: IsoDep): Boolean {
        ...
        while (currentAttempt < MAX_SEND_ATTEMPTS * totalChunks) {
            ...
            mainHandler.post {
                onTransferStatusChanged?.invoke(
                        context.getString(
                                R.string.sending_chunk,
                                acknowledgedChunks.size + 1,
                                totalChunks
                        )
                )
    ...
    private fun completeChunkedTransfer(isoDep: IsoDep): Boolean {
        ...
        // Notify completion
        mainHandler.post { onTransferCompleted?.invoke() }
    ...
    private fun handleChunkedTransferError(errorMessage: String) {
        ...
        // Notify error
        mainHandler.post { onTransferError?.invoke(errorMessage) }
    }
    ...
    private fun findNextChunkToSend(): Int {
        ...
        // If all chunks have been attempted at least once, check for any that haven't been
        // acknowledged
    ...
    fun startTransferRetryTimeout() {
        ...
        // Schedule the timeout
        transferRetryTimeoutHandler?.postDelayed(
                transferRetryTimeoutRunnable!!,
                transferRetryTimeoutMs
        )

        // Notify retry started
        mainHandler.post { onTransferRetryStarted?.invoke() }
    }
